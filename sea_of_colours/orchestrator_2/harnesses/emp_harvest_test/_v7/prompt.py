"""Board-fact formatters — the computation layer this harness reuses verbatim.

This fork's copy of the split V12 made: it forked v7's *feeding* layer (how
a prompt is assembled into sections) but kept its *computation* layer, the
pure functions that turn an agent view into text. That split is the whole
reason this file exists inside a harness named for a different version, and
it is a deliberate one — these formatters are battle-tested and the
assembler above them is not the thing that needed rewriting.

What lives here is only what ``emp_harvest_test/prompt.py`` still imports:
...
``emp_harvest_test/prompt.py``; if you are looking for section order, doctrine
