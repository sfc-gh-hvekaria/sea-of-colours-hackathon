"""cthvse_striker — night entry point (clean fork of v11; changes land in Section 2).
...
both on the inference API) is native and on by default; ``CTHVSE_STRIKER_SINGLE=1``
...
from sea_of_colours.orchestrator_2.harnesses.cthvse_striker._v7 import (
    chat_schema,
...
from sea_of_colours.orchestrator_2.harnesses.cthvse_striker import prompt as prompt_mod
from sea_of_colours.orchestrator_2.harnesses.cthvse_striker import (
...
# ── Agent identity ─────────────────────────────────────────────────────
INNER_AGENT_LABEL = "CTHVSE_STRIKER"
THINKER_AGENT_LABEL = "CTHVSE_STRIKER_THINKER"
...
def _split_on() -> bool:
    """v12 runs the contained split by default; ``CTHVSE_STRIKER_SINGLE=1`` disables."""
    return os.environ.get("CTHVSE_STRIKER_SINGLE", "0").strip().lower() not in (
        "1", "true", "yes",
    )
...
def _autofill_on() -> bool:
    ...
    AGENT spends the shortfall and stays the author. Set ``CTHVSE_STRIKER_AUTOFILL=1``
    to restore the old guarantee and measure the two against each other.
    """
    return os.environ.get("CTHVSE_STRIKER_AUTOFILL", "0").strip().lower() in (
        "1", "true", "yes", "on",
    )
